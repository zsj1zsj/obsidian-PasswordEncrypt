...
function fixture(notes, debounce = 20) {
  let reads = 0, namesRead = 0;
  const settings = { storageMode: 'secret-storage', keys: { 'test-key': { secretId: 'stored-id' } }, legacyKeyId: '' };
  const host = { paths: () => [...notes.keys()], read: async path => { reads++; if (!notes.has(path)) throw Error('private error'); return notes.get(path); },
  ...
  return { index, host, settings, reads: () => reads, namesRead: () => namesRead };
}
...
(async () => {
  ...
  try {
    ...
    index.changed('mixed.md'); index.refreshStatuses(); index.refresh();
    assert.equal(f.reads(), 0); assert.equal(f.namesRead(), 0); assert.equal(index.snapshot().active, false);
    ...
    // One debounced read for a burst of changes.
    const previous = f.reads();
    for (let i = 0; i < 8; i++) { notes.set('mixed.md', wrap(cipher)); index.changed('mixed.md'); }
    await idle(index); assert.equal(f.reads(), previous + 1);
    ...
    notes.set('mixed.md', wrap(cipher) + '\n' + wrap(cipher)); index.changed('mixed.md', undefined, false); await idle(index);
    ...
    raceNotes.set('renamed/b.md', 'new empty note'); f.index.changed('renamed/b.md', undefined, false);
    ...
    transitions.delete('x/a.md'); f.index.changed('x/a.txt', 'x/a.md', false); await idle(f.index);
    assert.equal(f.index.snapshot().notes.length, 1);
    transitions.set('x/a.md', wrap(cipher)); f.index.changed('x/a.md', 'x/a.txt', false); await idle(f.index);
    ...
    f.host.read = async () => { throw Error('SECRET EXCEPTION'); }; f.index.changed('error.md', undefined, false); await idle(f.index);
    ...
    const gate = deferred(); f.host.read = () => gate.promise; f.index.refresh(); f.index.changed('error.md');
