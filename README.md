...
When upgrading from version 0.1.0, a master password previously stored in `data.json` is migrated to SecretStorage and removed from the plugin settings. An encrypted verification record is created at the same time. Existing `EPB1` blocks remain readable and are not rewritten automatically.

For older configurations that already have a key but lack its verification record, inserting a new block first verifies the master password against an existing block for that key. This check can read notes outside the catalog's scan folders, then saves only an encrypted verification record for later operations. An incorrect password cannot silently create new blocks under the old key ID. If no matching block is available, use **Change master password** before inserting new blocks. Session and prompt modes do not read SecretStorage passwords for this verification.
...
- Browse blocks grouped by note path, with title, opening line, envelope version, key identifier, and status. Expand the key identifier to see its full value and parity setting.
- Search block titles, note paths or full key IDs, and filter by **All**, **Needs attention**, **EPB1**, or **EPB2**. Totals describe the catalog within the configured scope, not just the current search/filter. Issues count affected blocks plus unreadable/unscannable notes; a block with several problems counts once. Background updates preserve keyboard focus, expanded details, and scroll position when their target remains available.
...
The catalog starts scanning only when the panel is first opened (including restoration of an already-open panel). After activation it tracks Markdown note creation, edits, deletion, and file/folder renames, even if you close the panel. Individual file events update only their target; folder changes and full refreshes enumerate the relevant notes. Edits are debounced for 500 ms, with at most two background note scans at a time. Generation checks discard stale reads after edits, deletion, or renames. Results update progressively, with 100 visible entries per batch and **Load more** for larger catalogs. A refresh does not write to notes.

Only metadata is retained in the in-memory index: titles, paths, line positions, occurrence numbers, hashes/digests, versions, key identifiers, parity, and diagnostics. Note text and encrypted payloads are read transiently for scanning, but are not retained in the index. The catalog is never written to `data.json` or another cache file. Plugin unload clears the index and cancels queued work. The panel's metadata is visible to anyone able to use the running app; this feature is not an additional security boundary.

Separately, rendering block titles reuses a bounded in-memory cache of note source and parsed block positions. It refreshes when source text changes and clears on Lock or plugin unload. Decrypted passwords are never added to this cache. Editing a title uses a fresh scan and still checks for concurrent note changes before writing.
